...
var nAn_stochastics_module = function() {
     ...
     * the rest should be self-explanatory
     */
    this.milstein = function (A, D, Dy, initial, dt, t_final, parameters) {
        var d = initial.length;
        var N = Math.floor(t_final / dt);
        var result = new Float32Array(N * d);
        var t = new Float32Array(N);
        ...
        for (i = 0; i < N-1; i++) {
            var A_cur = A(y_cur, t[i], parameters)
            var D_cur = D(y_cur, t[i], parameters)
            var Dy_cur = Dy(y_cur, t[i], parameters)
            t[i+1] = (i+1) * dt;
            n = THIS.boxMuller2();
            for (j = 0; j < d; j++) 
                result[i + 1 + j * N] = y_cur[j] = 
     ...
     * the rest should be self-explanatory
     */
    this.colour = function (A, D, sigma, tau, initial, dt, t_final, parameters) {
        var d = initial.length;
        var N = Math.floor(t_final / dt);
        var result = new Float32Array(N * d);
        ...
        for (i = 0; i < N-1; i++) {
            ...
            var A_next = A(y_next, t[i], parameters)
            var D_next = D(y_next, t[i], parameters)
            for (j = 0; j < d; j++) {
